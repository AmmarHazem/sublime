...
# from sublime.qa.models import Answer
...
class Notification(models.Model):
    ...
    NOTIFICATION_TYPES = (
        ...
        (REPLY, _('replied to')),
        (STARTED_VIDEO_CALL, _('started video call')),
        )
    ...
    def __str__(self):
        if self.action_object:
            return f'{self.actor} {self.get_verb_display()} {type(self.action_object).__class__} {self.time_since()} ago'
...
def notification_handler(actor, recipient, verb, **kwargs):
    ...
    if recipient == 'global':
        ...
        notification_broadcast(actor, key, **kwargs)
        ...
        notification_broadcast(
            ...
            verb=verb,
            action_object = action_object,
            message_text = kwargs.pop('message_text', None),
            question_url = kwargs.pop('question_url', None),
            **kwargs,
        )
...
def notification_broadcast(actor, key, **kwargs):
    ...
    payload = {
            ...
            'notif_type' : action_object.__class__.__name__,
            **kwargs,
            # 'question_url' : kwargs.pop('question_url', None),
        }
    if not recipient:
        ...
        async_to_sync(channel_layer.group_send)(f'{recipient}-notification', payload)


def pre_save_notification(sender, instance, *args, **kwargs):
    if instance.action_object:
        instance.object_url = instance.action_object.get_absolute_url()

pre_save.connect(pre_save_notification, sender = Notification)
