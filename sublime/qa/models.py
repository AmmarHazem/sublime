...
from sublime.notifications.models import notification_handler, Notification
...
def post_save_answer(sender, instance, created, *args, **kwargs):
    if created:
        if instance.user != instance.question.user:
            notification_handler(
                actor = instance.user,
                recipient = instance.question.user,
                verb = Notification.ANSWERED,
                action_object = instance,
                id_value = instance.pk,
                message_text = f'{instance.user} posted an answer on your question.',
                question_url = instance.question.get_absolute_url(),
...
def post_save_vote(sender, instance, created, *args, **kwargs):
    if created:
        notification_handler(
            actor = instance.user,
            recipient = instance.vote.user,
            verb = Notification.VOTED,
            action_object = instance,
            id_value = instance.pk,
            question_url = instance.get_absolute_url(),
            voted_on = instance.vote.__class__.__name__,
            message_text = f'{instance.user} voted on your {instance.vote.__class__.__name__}',
            key = instance.__class__.__name__,
        )
